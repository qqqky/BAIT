...
public class Sha256Hash implements Serializable, Comparable<Sha256Hash> {
    ...
    private Sha256Hash(byte[] rawHashBytes) {
        UtilsGuava.checkArgument(rawHashBytes.length == LENGTH);
    ...
    public static Sha256Hash of(File file) throws IOException {
        try (FileInputStream in = new FileInputStream(file)) {
            return of(UtilsGuava.toByteArray(in));
        }
    }
    ...
    public int hashCode() {
        ...
        return UtilsGuava.fromBytes(bytes[LENGTH - 4], bytes[LENGTH - 3], bytes[LENGTH - 2], bytes[LENGTH - 1]);
    }
