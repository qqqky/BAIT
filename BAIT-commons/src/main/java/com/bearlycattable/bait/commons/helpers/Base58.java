...
public class Base58 {
    ...
    private final MessageDigest dig = loadDigester();
    ...
    public byte[] decodeChecked(String input) {
        ...
        byte[] withZeroRemoved = Arrays.copyOfRange(data, 1, data.length); //pubkeyhash
        return withZeroRemoved;
    }

    public byte[] decodeUnchecked(String input) {
        byte[] decoded  = decode(input);
        if (decoded.length < 4)
            throw new RuntimeException("Input too short: " + decoded.length);
    ...
    public MessageDigest loadDigester() {
        try {
