...
public abstract class BaseEncoding {
    ...
    public final String encode(byte[] bytes, int off, int len) {
        UtilsGuava.checkPositionIndexes(off, off + len, bytes.length);
    ...
    //     Objects.requireNonNull(encodedSink);
    //     return new ByteSink() {
    //         @Override
    ...
    //     Objects.requireNonNull(encodedSource);
    ...
    private static final class Alphabet {
        ...
        Alphabet(String name, char[] chars) {
            ...
            try {
                this.bitsPerChar = UtilsGuava.log2(chars.length, UNNECESSARY);
            ...
            byte[] decodabet = new byte[UtilsGuava.MAX + 1];
            Arrays.fill(decodabet, (byte) -1);
            for (int i = 0; i < chars.length; i++) {
                char c = chars[i];
                UtilsGuava.checkArgument(c < decodabet.length, "Non-ASCII character: %s", c);
                UtilsGuava.checkArgument(decodabet[c] == -1, "Duplicate character: %s", c);
                decodabet[c] = (byte) i;
            }
            ...
            for (int i = 0; i < bytesPerChunk; i++) {
                validPadding[UtilsGuava.divide(i * 8, bitsPerChar, CEILING)] = true;
            }
        ...
        Alphabet upperCase() {
            ...
            UtilsGuava.checkState(!hasUpperCase(), "Cannot call upperCase() on a mixed-case alphabet");
            char[] upperCased = new char[chars.length];
            for (int i = 0; i < chars.length; i++) {
                upperCased[i] = UtilsGuava.toUpperCase(chars[i]);
            }
        ...
        Alphabet lowerCase() {
            ...
            UtilsGuava.checkState(!hasLowerCase(), "Cannot call lowerCase() on a mixed-case alphabet");
            char[] lowerCased = new char[chars.length];
            for (int i = 0; i < chars.length; i++) {
                lowerCased[i] = UtilsGuava.toLowerCase(chars[i]);
            }
    ...
    static class StandardBaseEncoding extends BaseEncoding {
        ...
        int maxEncodedSize(int bytes) {
            return alphabet.charsPerChunk * UtilsGuava.divide(bytes, alphabet.bytesPerChunk, CEILING);
        }
        ...
        void encodeChunkTo(Appendable target, byte[] bytes, int off, int len) throws IOException {
            Objects.requireNonNull(target);
            UtilsGuava.checkPositionIndexes(off, off + len, bytes.length);
            UtilsGuava.checkArgument(len <= alphabet.bytesPerChunk);
            long bitBuffer = 0;
            for (int i = 0; i < len; ++i) {
        ...
        public InputStream decodingStream(final Reader reader) {
            ...
            return new InputStream() {
                ...
                public int read(byte[] buf, int off, int len) throws IOException {
                    ...
                    UtilsGuava.checkPositionIndexes(off, off + len, buf.length);
        ...
        @Override
        public int hashCode() {
            return alphabet.hashCode() ^ Objects.hash(paddingChar);
        }
    }
    ...
    static final class Base16Encoding extends StandardBaseEncoding {
        ...
        private Base16Encoding(Alphabet alphabet) {
            super(alphabet, null);
            UtilsGuava.checkArgument(alphabet.chars.length == 16);
            for (int i = 0; i < 256; ++i) {
    ...
    static final class Base64Encoding extends StandardBaseEncoding {
        ...
        private Base64Encoding(Alphabet alphabet, Character paddingChar) {
            ...
            UtilsGuava.checkArgument(alphabet.chars.length == 64);
        }
    ...
    static Reader ignoringReader(final Reader delegate, final String toIgnore) {
        ...
        Objects.requireNonNull(toIgnore);
        return new Reader() {
            @Override
            public int read() throws IOException {
                int readChar;
    ...
    static Appendable separatingAppendable(
        ...
        UtilsGuava.checkArgument(afterEveryChars > 0);
    ...
    static final class SeparatedBaseEncoding extends BaseEncoding {
        ...
        int maxEncodedSize(int bytes) {
            ...
            return unseparatedSize
                    + separator.length() * UtilsGuava.divide(Math.max(0, unseparatedSize - 1), afterEveryChars, FLOOR);
        }
