...
public final class AdvancedSearchHelperDecrementalWords extends AbstractAdvancedSearchHelper implements CustomKeyGenerator, PredictableEnd {

    private static final Logger LOG = Logger.getLogger(AdvancedSearchHelperDecrementalWords.class.getName());
    ...
    public AdvancedSearchHelperDecrementalWords(@NonNull AdvancedSearchHelperCreationContext creationContext) {
    ...
    public byte[] buildNextPrivBytes(byte[] current, List<Integer> disabledWords) {
        return modifierB.decrementPrivWordsB(current, disabledWords);
    }
    ...
    public @NonNull String predictLastItem(@NonNull String seed, int iterations, List<Integer> disabledWords) {
        return modifier.decrementWordsBy(seed, iterations, disabledWords);
    }
}
