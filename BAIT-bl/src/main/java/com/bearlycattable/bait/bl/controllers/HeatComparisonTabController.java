...
import com.bearlycattable.bait.commons.enums.PrivTypeEnum;
...
public class HeatComparisonTabController {
    ...
    private NumberFormatTypeEnum currentNumberFormatType;
    ...
    public void setMainController(HeatVisualizerController mainController) {
    ...
    private void insertComponentForReferencePK() {
        ...
        btn.setOnAction(event -> {
            String key = mainController.getCurrentInput();
        ...
        if (mainController != null) {
            DarkModeHelper.toggleDarkModeForComponent(mainController.isDarkMode(), parent);
        }

        comparisonVBoxReferenceKeyParent.getChildren().add(parent);
    }

    void setReferenceKey(String input, QuickSearchComparisonType type) {
        switch (type) {
            case COLLISION:
                comparisonRadioReferenceKeyTypePK.fire();
    ...
    void setCurrentKey(String input) {
        if (!HeatVisualizerConstants.PATTERN_SIMPLE_64.matcher(input).matches()) {
    ...
    private void insertComponentForReferencePKH() {
        ...
        btn.setOnAction(event -> {
            String PKH = mainController.getUnencodedPubFromConverterTab();
        ...
        btn2.setOnAction(event -> {
            ...
            removeMessage();
            mainController.setUnencodedPubInConverterTab(PKH);
        });
    ...
    private void removePubStats() {
        pubUncompressedHeatPositiveAbsoluteIndexes.keySet().stream().forEach(key -> {
        ...
        pubUncompressedHeatNegativeAbsoluteIndexes.keySet().stream().forEach(key -> {
        ...
        pubCompressedHeatPositiveAbsoluteIndexes.keySet().stream().forEach(key -> {
        ...
        pubCompressedHeatNegativeAbsoluteIndexes.keySet().stream().forEach(key -> {
        ...
        referenceForUncompressedAbsoluteIndexes.keySet().stream().forEach(key -> {
            referenceForUncompressedAbsoluteIndexes.get(key).setText("");
        });

        referenceForCompressedAbsoluteIndexes.keySet().stream().forEach(key -> {
    ...
    private boolean isReferenceKeyValid() {
        switch (getSelectedReferenceKeyTypeFromUi()) {
            case COLLISION:
    ...
    void insertPubSimilarityPercentLabels(PubComparisonResult heatResult, PubTypeEnum pubType) {
        ...
        switch (pubType) {
            case UNCOMPRESSED:
                labelPercentUncompressedPositive.setText(mainController.getSimilarityMappings().get(mapIndexForPositive).setScale(0, RoundingMode.HALF_UP).toString());
                labelPercentUncompressedNegative.setText(mainController.getSimilarityMappings().get(mapIndexForNegative).setScale(0, RoundingMode.HALF_UP).toString());
                break;
            case COMPRESSED:
                labelPercentCompressedPositive.setText(mainController.getSimilarityMappings().get(mapIndexForPositive).setScale(0, RoundingMode.HALF_UP).toString());
                labelPercentCompressedNegative.setText(mainController.getSimilarityMappings().get(mapIndexForNegative).setScale(0, RoundingMode.HALF_UP).toString());
                break;
            default:
    ...
    private int getResultForHeatType(PubComparisonResult currentResult, HeatOverflowTypeEnum heatType) {
        return mainController.getNormalizedMapIndexFromComparisonResult(HeatOverflowTypeEnum.HEAT_POSITIVE == heatType ? currentResult.getPositive() : currentResult.getNegative(), currentResult.getForScaleFactor());
    }
    ...
    void insertPubColorStylesToUi(int wordNumber, Map<Integer, Label> pubPositiveHeatMappings, Map<Integer, Label> pubNegativeHeatMappings, PubHeatResultWrapper resultWrapper) {
        ...
        pubPositiveHeatMappings.get(wordNumber).getStyleClass().add(mainController.getColorMappings().get(resultWrapper.getHeatPositive()));

        pubNegativeHeatMappings.get(wordNumber).getStyleClass().clear();
        pubNegativeHeatMappings.get(wordNumber).getStyleClass().add(mainController.getColorMappings().get(resultWrapper.getHeatNegative()));
    }
    ...
    void showPrivStats() {
        ...
        for (int i = 1; i <= 8; i++) {
            String wordInCurrentKeyField = readHexWordFromUi(i, PrivTypeEnum.CURRENT_KEY);
            ...
            getPrivHeat(i, wordInCurrentKeyField, currentNumberFormatType).ifPresent(result -> {
                ...
                NumberFormatTypeEnum numberFormat = comparisonChoiceBoxNumberFormatType.getSelectionModel().getSelectedItem();
                insertPrivNumericDifferenceLabel(justI, result, numberFormat);
            });
        }
    ...
    private Optional<PrivHeatResultWrapper> getPrivHeat(int wordNumber, String word, NumberFormatTypeEnum type) {
        String wordInReferenceKeyField = readHexWordFromUi(wordNumber, PrivTypeEnum.REFERENCE_KEY);
    ...
    private String readHexWordFromUi(int wordNumber, PrivTypeEnum type) {
    ...
    private void insertPrivNumericDifferenceLabel(int index, PrivHeatResultWrapper heatWrapper, NumberFormatTypeEnum numberFormatType) {
    ...
    private String convertToStringResult(NumberFormatTypeEnum targetType, long heatResult) {
        switch (targetType) {
            case DECIMAL:
    ...
    private void doImportPriv() {
        String currentInput = mainController.getCurrentInput();
    ...
    private void insertPrivColorStylesWithResolutionToUi(int wordNumber, int colorIndexPositive, int colorIndexNegative) {
        ...
        privHeatPositiveContainerMappings.get(wordNumber).getStyleClass().add(mainController.getColorMappings().get(colorIndexPositive));

        privHeatNegativeContainerMappings.get(wordNumber).getStyleClass().clear();
        privHeatNegativeContainerMappings.get(wordNumber).getStyleClass().add(mainController.getColorMappings().get(colorIndexNegative));
    }
