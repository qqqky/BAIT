...
public class QuickSearchTabController {
    ...
    private void exportPubUnencodedToConverterTab() {
        ...
        removeErrorMessage();
        mainController.setUnencodedPubInConverterTab(key);
    }

    private void importPubUnencodedFromConverterTab() {
        String unencodedPub = mainController.getUnencodedPubFromConverterTab();
    ...
    private void doQuickSearch() {
        ...
        SimpleSearchHelperCreationContext context = SimpleSearchHelperCreationContext.builder()
                .similarityMappings(mainController.getSimilarityMappings())
    ...
    public void showQuickSearchResults(@NonNull PubComparisonResultWrapper highest, int forAccuracy) {
        if (!highest.equalsEmpty()) {
            insertInfoMessage(rb.getString("info.searchCompletedForAccuracy") + forAccuracy
                    ...
                    + rb.getString("info.bestResultAndAccuracyFound") + highest.getCommonPriv() + " (" + mainController.getSimilarityMappings().get(helper.recalculateIndexForSimilarityMappings(highest.getHighestPoints(), getPubAccuracyScaleFactorFromUi()))  + ")"
                    + System.lineSeparator()
            ...
            if (quickSearchCbxMoveToHeatComparisonWhenSearchDone.isSelected()) {
                mainController.switchToComparisonTab();
            }
            return;
        }
    ...
    private void performSearchParallel(SimpleSearchHelper simpleSearchHelper, int accuracy) {
        ...
        QuickSearchContext quickSearchContext = QuickSearchContext.builder()
                ...
                .accuracy(accuracy)
                .verbose(mainController.isVerboseMode())
                .printSpacing(mainController.isVerboseMode() ? 1 : 0)
    ...
    private void displaySearchResults(PubComparisonResultWrapper highest) {
        ...
        highest.getResultAsOptionalForUncompressed().ifPresent(result -> mainController.insertSearchResultsToUiForUncompressed(result));
        highest.getResultAsOptionalForCompressed().ifPresent(result -> mainController.insertSearchResultsToUiForCompressed(result));

        mainController.setCurrentKeyInComparisonTab(highest.getCommonPriv());
        mainController.setReferenceKeyInComparisonTab(quickSearchTextFieldTargetKey.getText(), getQuickSearchTypeFromUi());
        mainController.setScaleFactorInComparisonTab(highest.getCommonScaleFactor());
    ...
    ScaleFactorEnum getPubAccuracyScaleFactorFromUi() {
