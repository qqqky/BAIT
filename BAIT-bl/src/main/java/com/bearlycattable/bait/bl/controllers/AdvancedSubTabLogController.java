...
public class AdvancedSubTabLogController {
    ...
    private static final int MAX_LOG_TEXT_SIZE = 50;
    ...
    private final List<LogText> unfilteredList = new ArrayList<>();
    ...
    public synchronized void log(String textPiece, Color color, int size, FontWeight weight, LogTextTypeEnum type) {
        ...
        if (isTypeDisabled(type)) {
            while (unfilteredList.size() >= MAX_LOG_ENTRIES) {
